...
namespace EshopOnAI.ProductGenerator
{
    public partial class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            var chatPrompt = """ 
                              Suggest 5 azure cloud native services. 
                              ...
                              Return a valid json array containing each suggestion with a random integer id [id], a name [name], and a list of merchandise items [merchandises] for that suggestion. Each merchandize has the service name [brand], name [name], description[description], type [type], and prompt[prompt]. Add random unique integer id [id], a decimal price [price] and an integer stock [availableStock] to each merchandise item.
                            """;
           ...
           Console.WriteLine("The number of choises is " + chatResponse.Choices.Count);
            ...
            foreach (Choice choice in chatResponse.Choices)
            {  
                //Console.WriteLine("---New Utem----");
                Console.WriteLine(choice.Text);
                var projects = JsonConvert.DeserializeObject<List<Root>>(choice.Text);
                ...
                foreach (var project in projects)
                {
                    ...
                    foreach (var merch in project.merchandises)
                    {
                ...
                string fileName = $"{DateTime.Now:yyyyMMddHHmmssffff}-project.json";
                string jsonFilePath = Path.Combine(imageFolderPath, fileName);
                File.WriteAllText(jsonFilePath, choice.Text);

                var brands = await catalogService.GetProjectsAsync(projects);
                //save brands as json into a file 
                fileName = $"{DateTime.Now:yyyyMMddHHmmssffff}-brands.json";
                jsonFilePath = Path.Combine(imageFolderPath, fileName);
                File.WriteAllText(jsonFilePath, JsonConvert.SerializeObject(brands));

                var types = await catalogService.GetCatalogTypesAsync(projects);
                //save types as json into a file
                fileName = $"{DateTime.Now:yyyyMMddHHmmssffff}-types.json";
                jsonFilePath = Path.Combine(imageFolderPath, fileName);
                File.WriteAllText(jsonFilePath, JsonConvert.SerializeObject(types));

                var items = await catalogService.GetCatalogItemsAsync(projects);
                //save items as json into a file
                fileName = $"{DateTime.Now:yyyyMMddHHmmssffff}-items.json";
                jsonFilePath = Path.Combine(imageFolderPath, fileName);
